...
- Collecte temps reel via Telnet: CPU, RAM, DHCP, WAN, latence, packet loss, uptime et etat local.
...
- Orchestration Airflow optionnelle pour entrainement et prediction planifies.
...
|---|---|---|
| Collecte | Interroger la HGW et construire des snapshots | `collector/data_collection.py`, `collector/telnet_client.py`, `collector/data_logger.py` |
...
```text
.
|-- collector/                  # Collecte Telnet et insertion DB
...
|-- docker-compose.airflow.yml  # Stack Airflow optionnelle
...
- Acces reseau a la HGW si la collecte Telnet est active.
...
| Predictor | `hgw_predictor` | - | Prediction periodique |
...
Airflow sert a orchestrer les taches planifiees:
...
En cas d'erreur de base Airflow non initialisee:
...
- Le collecteur actuel cible une HGW accessible en Telnet.
...
- Brancher les alertes SMS/e-mail sur les sorties `predictions_log`.
- Ajouter un service API REST pour exposer la prediction courante.
- Mettre en place un monitoring de drift et un retraining automatique via Airflow.
...
```text
Collecte Telnet -> TimescaleDB -> Feature engineering -> CatBoost/LSTM -> Alertes -> Grafana/Notebook
```
